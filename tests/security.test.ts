...
it("rejects cross-origin writes even with valid cookie and CSRF", async () => {
  expect(
    (await post("/api/projects", {}, { origin: "https://attacker.invalid" }))
      .status,
  ).toBe(403);
});
...
it("requires session, CSRF, JSON and explicit Origin for writes", async () => {
  expect((await post("/api/projects", {}, { "x-csrf-token": "" })).status).toBe(
    403,
  );
  expect((await post("/api/projects", {}, { cookie: "" })).status).toBe(401);
  expect((await post("/api/projects", {}, { origin: "" })).status).toBe(403);
  expect(
    (await post("/api/projects", {}, { "content-type": "text/plain" })).status,
  ).toBe(415);
});
it("does not expose transcripts before explicit server-side selection", async () => {
  expect(
    (
      await fetch(`${origin}/api/history?key=demo%3Aharbor`, {
        headers: { cookie },
      })
    ).status,
  ).toBe(403);
  await post("/api/select", { key: "demo:harbor" });
  const response = await fetch(`${origin}/api/history?key=demo%3Aharbor`, {
    headers: { cookie },
  });
...
it("caps history page size and disallows arbitrary Gateway RPCs", async () => {
  ...
  expect(
    (
      await fetch(`${origin}/api/history?key=demo%3Aharbor&limit=10000`, {
        headers: { cookie },
      })
    ).status,
  ).toBe(400);
  expect((await post("/api/rpc", { method: "config.set" })).status).toBe(404);
});
